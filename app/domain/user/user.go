...
func newUser(
	...
	if _, ok := phoneNumberDigitMap[utf8.RuneCountInString(phoneNumber)]; !ok {
		return nil, domainErr.NewError("電話番号の値が不正です。")
	}
...
func newAddress(
	...
	if prefecture == "" || city == "" || extra == "" {
		return address{}, domainErr.NewError("住所の値が不正です。")
	}
	return address{
